...
int main (void) {
    ...
    int function = 0; // 0 = none, 1 = increase by one, 2 = decrease by one (til 0), 3 = increase by two
    ...
    while (fscanf(fp, "%[^\n] ", str) != EOF) {
      ...
      for (int y = y_start; y <= y_end; y++) {
        for (int x = x_start; x <= x_end; x++) {
          if (function == 1) {grid [x][y]++;} // increase by one
          else if (function == 2) {if (grid [x][y] != 0) {grid [x][y]--;}} // decrease by one (til 0)
          else if (function == 3) {grid [x][y] = grid [x][y] + 2;} // increase by two
        }
      }
    processed++;
    } // while
    ...
    printf("\nTotal brightness: %d    %d lines processed\n", counter, processed);
    fclose(fp);
    return 0;
} //main
